...
void CollisionManager::onTick(float seconds)
{
    ...
    foreach(Entity *entity, m_entities)
    {
        ...
        float noise = world->planet->getNoise(glm::normalize(ent->getPosition()));

        if(glm::length(ent->getPosition()) - 1.f - ent->getRadius() * 0.25 < noise)
        {
            ent->setPosition(glm::normalize(ent->getPosition())
                             * (1.f + noise + ent->getRadius() * 0.25f));
